...
class CalcCtrl {
	...
	public function process(){
		...
		if ($this->validate()) {
            ...
            if ($this->form->z <= 0) {
                ...
                $result = ($this->form->x * (($this->form->z / 100) / 12) * ((1 + (($this->form->z / 100) / 12)) ** ($this->form->y * 12))) / (((( 1 + ($this->form->z / 12 / 100)) ** ($this->form->y * 12))) - 1);
            }
    ...
    public function generateHomePage(){
        ...
        getSmarty()->display('Home.tpl'); // już nie podajemy pełnej ścieżki - foldery widoków są zdefiniowane przy ładowaniu Smarty
    }
}
